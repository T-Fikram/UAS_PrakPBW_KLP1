...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Registrasi gagal. Periksa kembali inputan Anda.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen mesh-gradient flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute top-10 right-[15%] w-16 h-16 rounded-2xl bg-emerald-400/20 dark:bg-emerald-500/10 blur-sm animate-float" />
      <div className="absolute bottom-20 left-[15%] w-24 h-24 rounded-full bg-sky-400/20 dark:bg-sky-500/10 blur-sm animate-float" style={{ animationDelay: '1.5s' }} />
      <div className="absolute top-[60%] right-[8%] w-12 h-12 rounded-xl bg-purple-400/20 dark:bg-purple-500/10 blur-sm animate-float" style={{ animationDelay: '3s' }} />
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md z-10 animate-fade-in">
        ...
        <p className="mt-2 text-center text-sm text-slate-500 dark:text-slate-400">
          Buat akun HealthMirror Anda secara gratis
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10 px-4 sm:px-0">
        <Card className="animate-scale-in shadow-xl border-slate-200/50 dark:border-slate-800">
          ...
          <div className="mt-6 text-center text-sm">
            <span className="text-slate-500 dark:text-slate-400">Sudah punya akun? </span>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-center gap-2 text-xs text-slate-400 dark:text-slate-500 font-semibold">
            ...
            <span>Bergabunglah dengan 1.240+ pengguna lainnya</span>
          </div>
        </Card>
      </div>
    </div>
